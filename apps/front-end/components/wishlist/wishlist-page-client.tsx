	...
	return (
		<div>
							...
							>
								...
								<span
									className={clsx(
										...
										index === activeTab ? "bg-brand-100 text-brand" : "bg-gray-100 text-gray-600",
									)}
								>
									{wl.items?.length ?? 0}
								</span>
			...
			<div className="mt-6 flex items-center justify-between">
				<h2 className="text-lg font-medium text-gray-900">{activeWishlist?.name || "My Wishlist"}</h2>
				<div className="flex items-center gap-3">
					<WishlistActionsMenu wishlist={activeWishlist!} onDeleted={() => setActiveTab(0)} />
			...
			{activeWishlist && activeWishlist.items?.length > 0 ? (
				<div className="mt-8 grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-3 xl:gap-x-8">
					{activeWishlist.items.map((item) => (
						<WishlistItemCard key={item.id} item={item} wishlistId={activeWishlist.id} />
					))}
				</div>
			) : (
				<EmptyState />
...
function WishlistItemCard({ item, wishlistId }: { item: WishlistItem; wishlistId: string }) {
	...
	return (
		...
		>
			...
			{/* Product image */}
			{thumbnail ? (
				<Link href={product?.handle ? `/product/${product.handle}` : "#"} prefetch={true}>
					<Image
						src={thumbnail}
			...
			<div className="flex flex-1 flex-col space-y-2 p-4">
				...
				{variant?.title && variant.title !== "Default" && <p className="text-sm text-gray-500">{variant.title}</p>}
...
function EmptyState() {
	return (
		<div className="py-16 text-center">
			...
			<h3 className="mt-4 text-sm font-semibold text-gray-900">No saved items yet</h3>
			<p className="mt-2 text-sm text-gray-500">Start browsing and save the products you love.</p>
			<div className="mt-6">
				<Link
					href="/products"
...
function NewWishlistButton() {
	...
	const [state, formAction, isPending] = useActionState<WishlistActionResult, FormData>(async (prev, formData) => {
...
function WishlistActionsMenu({ wishlist, onDeleted }: { wishlist: Wishlist; onDeleted: () => void }) {
...
function RenameWishlistDialog({ wishlist, open, onClose }: { wishlist: Wishlist; open: boolean; onClose: () => void }) {
	const { showNotification } = useNotification();
	const [state, formAction, isPending] = useActionState<WishlistActionResult, FormData>(async (prev, formData) => {
				...
				<div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
					...
					>
						...
						<p className="mt-2 text-sm text-gray-500">
							...
							{wishlist.name || "this wishlist"}&rdquo;? This action cannot be undone.
						</p>
						<div className="mt-5 flex justify-end gap-3">
							<button
								type="button"
