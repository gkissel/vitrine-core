...
function buildContentSecurityPolicy(): string {
	...
	const backendOrigin = getCspOrigin(process.env.MEDUSA_BACKEND_URL, isDev ? "http://localhost:9000" : "");
	const meilisearchOrigin = getCspOrigin(process.env.NEXT_PUBLIC_MEILISEARCH_HOST);
	const posthogOrigin = getCspOrigin(process.env.NEXT_PUBLIC_POSTHOG_HOST);

	if (!isDev && !backendOrigin) {
		console.warn("[next.config] MEDUSA_BACKEND_URL is not set - CSP will block backend requests");
	}
