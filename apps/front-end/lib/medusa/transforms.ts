...
import type { Cart, CartItem, Collection, Image, Money, Product, ProductOption, ProductVariant } from "lib/types";
...
function transformImage(image: HttpTypes.StoreProductImage, fallbackAlt: string): Image {
	return {
		url: image.url || "",
		altText: fallbackAlt,
		width: 0,
...
function transformVariant(
	...
	const availableForSale = !manageInventory || inventoryQuantity == null || inventoryQuantity > 0;
...
export function transformProduct(product: HttpTypes.StoreProduct): Product {
	...
	return {
		...
		seo: {
			...
			description: (product.metadata?.seo_description as string) || product.description || "",
		},
		tags,
...
export function transformCollection(collection: HttpTypes.StoreCollection): Collection {
	return {
		...
		seo: {
			title: (collection.metadata?.seo_title as string) || collection.title || "",
			description:
				(collection.metadata?.seo_description as string) || (collection.metadata?.description as string) || "",
		},
