...
export const validateEnvironmentVariables = () => {
	const requiredEnvironmentVariables = ["MEDUSA_BACKEND_URL", "NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY"];
...
export const transformProductToTailwind = (product: Product): TailwindProduct => {
	...
	const firstColorOption = product.variants[0]?.selectedOptions.find((option) => option.name.toLowerCase() === "color");
	const defaultColor = firstColorOption?.value || "";

	// Format price with currency symbol
	const price = product.variants[0]?.price ? `$${parseFloat(product.variants[0].price.amount).toFixed(2)}` : "$0.00";
...
export const transformCollectionToTailwind = (collection: Collection): TailwindCollection => {
	return {
		name: collection.title,
		description: collection.description || collection.seo.description,
		imageSrc: collection.image?.url || PLACEHOLDER_IMAGE_SRC,
...
export const transformProductToTailwindDetail = (product: Product, averageRating?: number): TailwindProductDetail => {
	...
	const colorVariants = product.variants
		...
		.filter((item): item is { value: string; variant: Product["variants"][number] } => Boolean(item));
	...
	const details = [
		{
			name: "Features",
			items: ["Premium quality materials", "Durable construction", "Modern design", "Versatile usage"],
		},
		{
			name: "Care",
			items: ["Spot clean as needed", "Handle with care", "Store in a cool, dry place", "Avoid direct sunlight"],
		},
		{
			name: "Shipping",
			items: [
...
export const transformProductsToRelatedProducts = (products: Product[]): TailwindRelatedProduct[] => {
	return products.slice(0, 4).map((product, index) => {
		...
		const colorOption = colorVariant?.selectedOptions.find((option) => option.name.toLowerCase() === "color");
