...
async function seedRefundReasons(container: ExecArgs["container"], query: GraphQuery) {
	...
	const data = refundReasonsPtBR.filter((reason) => !existingCodes.has(reason.code));
...
export default async function seedDemoData({ container }: ExecArgs) {
	...
	if (!publishableApiKey) {
		...
		publishableApiKey = assertExists(data[0], "API key publicável não foi criada.");
	}
		...
		input: {
			collections: [
				...
				},
				{
					...
					handle: "luxo-gaucho",
					metadata: {
						description: "Erva-mate premium de seleção especial. Sabor marcante e tradição gaúcha em cada cuia.",
						origin: "Aurea, RS",
					},
				},
				{
	...
	const productsData = [
		// Seiva Verde
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.seivaVerdeNativa[0],
			images: ERVA_MATE_IMAGES.seivaVerdeNativa,
		...
		},
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.seivaVerdeSuave[0],
			images: ERVA_MATE_IMAGES.seivaVerdeSuave,
			options: [{ title: "Peso", values: ["500g", "1kg"] }],
			variants: [
				{
					title: "500g",
		...
		},
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.seivaVerdeVacuo[0],
			images: ERVA_MATE_IMAGES.seivaVerdeVacuo,
		...
		// Luxo Gaúcho
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.luxoGauchoPremium[0],
			images: ERVA_MATE_IMAGES.luxoGauchoPremium,
			options: [{ title: "Peso", values: ["500g", "1kg"] }],
			variants: [
				{
					title: "500g",
			...
			metadata: buildProductMetadata({
				ptBR: {
					title: "Erva-Mate Luxo Gaúcho Premium",
					description: "Erva-mate premium de seleção especial Luxo Gaúcho. Sabor marcante no começo e suave ao final.",
				},
				en: {
					title: "Luxo Gaúcho Premium Yerba Mate",
					description: "Premium selected yerba mate Luxo Gaúcho. Strong flavor at first and smooth at the end.",
				},
				es: {
		...
		},
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.luxoGauchoTradicional[0],
			images: ERVA_MATE_IMAGES.luxoGauchoTradicional,
			options: [{ title: "Peso", values: ["500g", "1kg"] }],
			variants: [
				{
					title: "500g",
		...
		// Goma Verde
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.gomaVerdeNativa[0],
			images: ERVA_MATE_IMAGES.gomaVerdeNativa,
			options: [{ title: "Peso", values: ["500g", "1kg"] }],
			variants: [
				{
					title: "500g",
		...
		},
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.gomaVerdePremium[0],
			images: ERVA_MATE_IMAGES.gomaVerdePremium,
			options: [{ title: "Peso", values: ["1kg"] }],
			variants: [
				{
					title: "1kg",
		...
		// Compostos
		{
			...
			weight: 1000,
			thumbnail: ERVA_MATE_IMAGES.compostaMenta[0],
			images: ERVA_MATE_IMAGES.compostaMenta,
			options: [{ title: "Peso", values: ["500g", "1kg"] }],
			variants: [
				{
					title: "500g",
			...
			metadata: buildProductMetadata({
				...
				en: {
					...
					description: "Refreshing and aromatic yerba mate blend with mint. Ideal for daily consumption.",
				},
				es: {
		...
		// Tereré
		{
			...
			weight: 500,
			thumbnail: ERVA_MATE_IMAGES.terere[0],
			images: ERVA_MATE_IMAGES.terere,
			...
			metadata: buildProductMetadata({
				ptBR: {
					title: "Erva-Mate para Tereré Natural",
					description: "Erva-mate especial para tereré. Sabor natural refrescante, perfeita para dias quentes.",
				},
				en: {
					title: "Natural Tereré Yerba Mate",
					description: "Special yerba mate for tereré. Refreshing natural flavor, perfect for hot days.",
				},
				es: {
					title: "Yerba Mate para Tereré Natural",
					description: "Yerba mate especial para tereré. Sabor natural refrescante, perfecta para días calurosos.",
				},
			}),
		},
		// Acessórios
		{
			...
			weight: 1500,
			thumbnail: ERVA_MATE_IMAGES.kitChimarrao[0],
			images: ERVA_MATE_IMAGES.kitChimarrao,
			...
			metadata: buildProductMetadata({
				ptBR: {
					title: "Kit Chimarrão Iniciante",
					description: "Kit completo para iniciar no chimarrão com cuia, bomba e erva-mate.",
				},
				en: {
					title: "Beginner Chimarrão Kit",
					description: "Complete starter kit for chimarrão with gourd, straw and yerba mate.",
				},
				es: {
					title: "Kit Inicial de Chimarrão",
					description: "Kit completo para empezar con el chimarrão con mate, bombilla y yerba mate.",
				},
			}),
		},
	];
	...
	for (let i = 0; i < productsData.length; i += BATCH_SIZE) {
		...
		logger.info(`  Criando lote de produtos ${batchNum}/${totalBatches} (${batch.length} produtos)...`);
