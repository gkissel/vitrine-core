...
const createOrderFromCartStep = createStep(
  ...
  async (input: WorkflowInput, { container }) => {
    ...
    const items = (cart.items ?? []).map((item) => {
      ...
      const fallbackUnit =
        quantity > 0 ? Number(item.total ?? 0) / quantity : 0;
      return {
        title: item.title ?? "",
        quantity,
    ...
    const order = await orderModuleService.createOrders({
      ...
      region_id: (cart as unknown as Record<string, unknown>).region_id as
        | string
        | undefined,
      sales_channel_id: (cart as unknown as Record<string, unknown>)
        .sales_channel_id as string | undefined,
      status: "pending",
    ...
    try {
      ...
      if (logger && typeof logger.warn === "function") {
        logger.warn(
          { err },
          "Failed to mark cart as completed after creating order",
        );
      }
    }
...
export const createOrderFromCartWorkflow = createWorkflow(
